...
import { optimism, arbitrum, polygon, sepolia, baseSepolia } from "viem/chains";
import {
  IconWallet,
  IconArrowRight,
  IconLoader,
...
import { MOCK_MERCHANT_ADDRESS, MOCK_USDC_TOKEN } from "@/constants/address";
...
export default function PaymentPage() {
  ...
  const { fundChannel } = useNitrolite(
  ...
  const invoice = {
    ...
    amount: "10",
    currency: "USDC",
    recipient: MOCK_MERCHANT_ADDRESS,
    details: "Web Design Services",
    targetChainId: baseSepolia.id,
    targetToken: MOCK_USDC_TOKEN,
  };
  ...
  const [status, setStatus] = useState<
    "idle" | "quoting" | "paying" | "success" | "error"
  >("idle");
              ...
              >
                ...
                {tokenList.map((t) => (
                  <option key={t.address} value={t.address}>
                    {t.symbol} ({Number(t.priceUSD).toFixed(2)})
                  </option>
                ))}
              </select>
            </div>
              ...
              <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                <div className="flex justify-between items-center text-sm mb-2">
                  ...
                  <span className="font-mono">
                    0.01{" "}
                    {tokenList.find((t) => t.address === sourceToken)?.symbol ||
                      "ETH"}
                  </span>
                </div>
                <div className="flex justify-center my-2 text-[var(--muted-foreground)]">
                  <IconArrowDownLeft size={16} />
                  {/* Using ArrowDown for flow */}
                </div>
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-mono font-bold text-green-400">
                    ...
                    {quote.estimate?.toAmount
                      ? (Number(quote.estimate.toAmount) / 1000000).toFixed(2)
                      : "..."}{" "}
                    USDC
                  </span>
                </div>
                <div className="mt-3 text-xs text-[var(--muted-foreground)] flex justify-between">
                  <span>
                    Gas Cost: ~$
                    {quote.estimate?.gasCosts?.[0]?.amountUSD || "0.00"}
                  </span>
                  <span>
                    Time: ~{Math.ceil(quote.estimate?.executionDuration / 60)}{" "}
                    min
                  </span>
                </div>
              </div>
            ...
            {status === "success" && (
              <div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4 text-center">
                ...
                <p className="text-xs text-[var(--muted-foreground)] mt-1">
                  Funds are settling in the Merchant's account.
                </p>
              </div>
            )}
